...
//    $amount = $item['amount'];
//    $cid = $item['amount'];
//    $desc = "[RP]" . $item['description'];
//    $reccu = $item['recurent_bill'];
//    $uid = $item['user_id'];
//    $next_update = $item['date_time'];
//
//    $sql_p1 = "INSERT INTO transactions (account_id, amount, exp_inc, category_id, `description`, recurent_bill, user_id, next_update)
//    VALUES ($aid, $amount, 'exp', $cid , 'RP', 1, 1, '2017-11-06');";
//    $stmt = $pdo->prepare($sql_p1);
//    $stmt->execute();
    //echo $item['date_time']."<br>". PHP_EOL;
    echo $item['account_id']."<br>". PHP_EOL;
    $time = $item['date_time'];
    $newd = date('Y-m-d', strtotime($time));
    echo $newd."<br>". PHP_EOL;
}
//echo $item['account_id']."<br>". PHP_EOL;
//echo $item['date_time']."<br>". PHP_EOL;